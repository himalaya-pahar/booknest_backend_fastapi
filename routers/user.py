...
@router.put('/')
def update_user(request: schemas.UserUpdate, db: d_b.SessionDep, current_user=Depends(oauth2.get_current_user)):
    return repo_user.update_user(request, db, current_user)